/**
 * MCP Server function for Creates a policy.
 */
...
class Post_V2_ParentMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_V2_ParentHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("policyId")) {
            queryParams.add("policyId=" + args.get("policyId"));
        }
        ...
        if (args.containsKey("rules")) {
            queryParams.add("rules=" + args.get("rules"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_v2_parent" + queryString;
    ...
    public static MCPServer.Tool createPost_V2_ParentTool(MCPServer.APIConfig config) {
        ...
        parentProperty.put("description", "Required. The resource that the policy is attached to, along with the kind of policy to create. Format: `policies/{attachment_point}/denypolicies` The attachment point is identified by its URL-encoded full resource name, which means that the forward-slash character, `/`, must be written as `%2F`. For example, `policies/cloudresourcemanager.googleapis.com%2Fprojects%2Fmy-project/denypolicies`. For organizations and folders, use the numeric ID in the full resource name. For projects, you can use the alphanumeric or the numeric ID.");
        properties.put("parent", parentProperty);
        Map<String, Object> policyIdProperty = new HashMap<>();
        policyIdProperty.put("type", "string");
        policyIdProperty.put("required", false);
        policyIdProperty.put("description", "The ID to use for this policy, which will become the final component of the policy's resource name. The ID must contain 3 to 63 characters. It can contain lowercase letters and numbers, as well as dashes (`-`) and periods (`.`). The first character must be a lowercase letter.");
        properties.put("policyId", policyIdProperty);
        ...
        return new MCPServer.Tool(definition, getPost_V2_ParentHandler(config));
    }
