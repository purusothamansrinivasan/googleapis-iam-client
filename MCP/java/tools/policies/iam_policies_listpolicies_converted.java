/**
 * MCP Server function for Retrieves the policies of the specified kind that are attached to a resource. The response lists only policy metadata. In particular, policy rules are omitted.
 */
...
class Get_V2_ParentMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_V2_ParentHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("prettyPrint")) {
            queryParams.add("prettyPrint=" + args.get("prettyPrint"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_v2_parent" + queryString;
    ...
    public static MCPServer.Tool createGet_V2_ParentTool(MCPServer.APIConfig config) {
        ...
        pageTokenProperty.put("description", "A page token received in a ListPoliciesResponse. Provide this token to retrieve the next page.");
        ...
        pageSizeProperty.put("description", "The maximum number of policies to return. IAM ignores this value and uses the value 1000.");
        ...
        return new MCPServer.Tool(definition, getGet_V2_ParentHandler(config));
    }
