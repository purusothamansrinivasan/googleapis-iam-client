...
func Iam_policies_createpolicyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIam_policies_createpolicyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_parent",
		mcp.WithDescription("Creates a policy."),
		...
		mcp.WithString("policyId", mcp.Description("The ID to use for this policy, which will become the final component of the policy's resource name. The ID must contain 3 to 63 characters. It can contain lowercase letters and numbers, as well as dashes (`-`) and periods (`.`). The first character must be a lowercase letter.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Iam_policies_createpolicyHandler(cfg),
	}
}
