...
func Iam_policies_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["etag"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("etag=%v", val))
		}
...
func CreateIam_policies_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v2_name",
		mcp.WithDescription("Deletes a policy. This action is permanent."),
		...
		mcp.WithString("etag", mcp.Description("Optional. The expected `etag` of the policy to delete. If the value does not match the value that is stored in IAM, the request fails with a `409` error code and `ABORTED` status. If you omit this field, the policy is deleted regardless of its current `etag`.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Iam_policies_deleteHandler(cfg),
	}
}
